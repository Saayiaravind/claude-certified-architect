...
- **`[x]` means taught in conversation, not researched by the teacher.** L0 was marked done
  purely from repo exploration, with the actual content never delivered to the learner in
  chat — caught when they asked "what was L0" after L4. Never mark a lesson complete based on
  having prepared the material; only mark it once it was actually walked through with the
  learner in that session.
...
| Date | Lesson(s) | Notes |
|---|---|---|
| 2026-09-16 | L0 | Tracker created; repo explored (guide_en.md, ~2600 lines: Part I 13 theory chapters, Part II 5 exam domains, worked questions, practice test); 36-lesson plan drafted. **Correction:** L0 was marked done here from repo exploration alone — the actual orientation content (exam format/scoring table, 5 domain weights, 8 scenarios) was never walked through with the learner until they asked "what was L0" after L4. Delivered properly then. **Process lesson:** marking a lesson `[x]` must mean it was taught in the conversation, not that the teacher researched it — applies at the lesson-tracking level same as the L1 full-coverage rule applies within a lesson. |
